...
import { grantEntitlement, revokeEntitlement, PREMIUM_KEY } from '@/server/entitlements';
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  grantEntitlement({ userId: id, key: PREMIUM_KEY, source: 'admin', createdByUserId: guard.value.user.id });
  auditLog({ actorUserId: guard.value.user.id, action: 'admin.premium_granted', targetUserId: id });
  return json({ ok: true });
}
